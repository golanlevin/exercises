...
var criticisms = [
  ...
  "No!",
  "Stop!",
  "Stop touching your face right now!",
]
...
function gotResults(err, result) {
  ...
  currState = classes[result.classIndex];
...
function updateCounts() {
  ...
  for (var i = 0; i < classes.length; i++){
    select('#add'+classes[i]).html(
      `➕ ${classes[i]} (${counts[classes[i]] || 0})`
    );
  } 
}
...
function voiceover(){
  ...
  var pool = currState == "TOUCHING" ? criticisms : compliments;
  var line = pool[~~(Math.random()*pool.length)];
  // synthesize with text to speech.

  if (currState != prevState){
    // if user changed from touching to not touching or vice verca,
    // we interrupt previous compliment/criticsm immediately
...
function draw(){
  ...
  for (var i = 0; i < classes.length; i++){
    var conf = confidences[classes[i]] || 0;
    fill(0,0,255);
    noStroke();
    rect(width-200,i*20+8,conf*200,16);
    fill(255);
    text(classes[i]+" "+(~~(conf*1000)/10)+"%",width-200,i*20+20);
  }
  if (currState == "TOUCHING"){
    push();
    fill(255,0,0,(sin(frameCount/5)*0.5+0.5)*100+50)
